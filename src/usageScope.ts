 ...
 * 无 sessionId 的桶也会进 client/model/day，避免丢数；项目维度仍需 session 映射。
 *
 * sessionsByModel：该模型键下「有用量的去重会话数」——按小时桶 model+sessionId，
 * 不用 session 表最后一次 model（避免裸 deepseek-v4-pro 与会话 · max 键分裂成 0 token 行）。
 */
export function buildHourlyDimTotals(
  ...
  byModel: DimTotals;
  byDay: DimTotals;
  /** 模型键 → 区间内出现过该 model 的去重 session 数 */
