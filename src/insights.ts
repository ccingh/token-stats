...
function isCountable(s: SessionRecord): boolean {
  if (s.dedupExcluded) return false;
...
export function computeLifetimeInsights(
  ...
  const byHour = new Map<number, number>(); // 0–23
  ...
  // 若有 hourly，lifetime tokens 以日汇总更贴近真实发生量
  if (hourly?.length) {
  ...
  const weekdayTotals = [0, 0, 0, 0, 0, 0, 0]; // Mon-first
  for (const [day, tokens] of byDay) {
    const js = parseDay(day).getDay(); // Sun=0
    const mon = (js + 6) % 7;
